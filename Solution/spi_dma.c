...
static int omap2_mcspi_request_dma(struct spi_device *spi)
{
	...
	mcspi = spi_controller_get_devdata(master);
	...
	mcspi_dma->dma_rx = dma_request_chan(&master->dev, mcspi_dma->dma_rx_ch_name);
	...
	// TODO 4.1.2: Request DMA channel for TX with dma_request_chan 
	// and assign it to dma_tx of mcspi_dma
	mcspi_dma->dma_tx = dma_request_chan(&master->dev, mcspi_dma->dma_tx_ch_name);
				...
				struct dma_slave_config cfg)
{
	...
	dmaengine_slave_config(mcspi_dma->dma_tx, &cfg);
		...
		if (tx) {
			// TODO 4.1.8: Register the Tx callback by assigning to callback_param
			// and callback fields of descriptor 
				...
				struct dma_slave_config cfg)
{
	...
	if (mcspi_dma->dma_rx) {
		...
		if (tx) {
			// TODO 4.1.13: Register the Rx callback by assigning appropriate values
			tx->callback_param = spi;
			tx->callback = omap2_mcspi_rx_callback; 
			// to callback_param and callback fields of descriptor
...
omap2_mcspi_txrx_dma(struct spi_device *spi, struct spi_transfer *xfer)
{
	...
	if (mcspi_dma->dma_tx && xfer->tx_buf != NULL) {
		...
		xfer->tx_dma = dma_map_single(mcspi->dev, (void *)xfer->tx_buf, /*buffer*/ xfer->len, DMA_TO_DEVICE);
	...
	if (mcspi_dma->dma_rx && xfer->rx_buf != NULL) {
		...
		xfer->rx_dma = dma_map_single(mcspi->dev, xfer->rx_buf, /*buffer*/ xfer->len, DMA_FROM_DEVICE);
	...
	// TODO 4.1.5: Initialize the DMA configuration:
	// src address to mcspi->phys + OMAP2_MCSPI_RX0 register (Use Physical address of RX0)
	// dst address to mcspi->phys + OMAP2_MCSPI_TX0 register (Use Physical address of TX0)
	// src and dst address width
	// Max burst for src and dst
	//cfg.src_addr, cfg.dst_addr, cfg.src_addr_width, cfg.dst_addr_width
    //cfg.src_maxburst, cfg.dst_maxburst
	cfg.src_addr = mcspi->phys + OMAP2_MCSPI_RX0;	
	cfg.dst_addr = mcspi->phys + OMAP2_MCSPI_TX0;	
...
static int spi_transfer_one_message(struct spi_controller *master,
		struct spi_message *m)
{
	struct omap2_mcspi *mcspi = spi_controller_get_devdata(master);
	...
	list_for_each_entry(t, &m->transfers, transfer_list) {
		...
		if (t->len) {
			...
			if ((mcspi_dma->dma_rx && mcspi_dma->dma_tx) &&
					spi_xfer_is_dma_mapped(master, spi, t) &&
					master->can_dma(master, spi, t))
...
static bool omap2_mcspi_can_dma(struct spi_controller *master,
...
static int my_mcspi_probe(struct platform_device *pdev)
{
	...
	master = spi_alloc_host(&pdev->dev, sizeof *mcspi);
	...
	mcspi->dev = &pdev->dev;
	mcspi->dma_channels = devm_kcalloc(&pdev->dev, master->num_chipselect, sizeof(struct omap2_mcspi_dma), GFP_KERNEL);
...
static void my_mcspi_remove(struct platform_device *pdev)
{
	struct spi_controller *master;
	struct omap2_mcspi *mcspi;
...
MODULE_AUTHOR("Embedded Trainings <info@embitude.in");
